...
import { getMonitorWebServiceStatus } from "../../../services/monitor-service/monitor-webservice-status-service";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import Pagination from "../../pagination/Pagination";

function MonitorWebserviceStatusList() {
    ...
    getMonitorWebServiceStatus()
      .then((response) => {
        ...
        if(monitorStatusList && monitorStatusList >0){
          showSuccessToast("Successfully fetched all Monitor webservice Status items.");
        }
        else{
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <CRow>
              <CCol xs={11}>
                <h4>Webservice Status Detail</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
            <p className="text-medium-emphasis small">
              List of all Webservice Status Details
            </p>
            {monitorStatusList && monitorStatusList.length === 0 ? (
              <div>No Webservice Status...</div>
            ) : (
              <CTable hover>
                <CTableHead>
                  <CTableRow>
                    <CTableHeaderCell scope="col">Sl No.</CTableHeaderCell>
                    <CTableHeaderCell scope="col">Webservice</CTableHeaderCell>
...
export default MonitorWebserviceStatusList;
